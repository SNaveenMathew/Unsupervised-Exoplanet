...
├── pipeline.R                          # LSTM autoencoder training, plotting, SQLite output
├── util.R                              # helpers (interpolation, phase folding, plots)
├── download.R                          # downloads Kepler KOI light curves into data/
├── counts.sh / counts_df.sh            # star / planet-count helpers for SQLite
...
├── plots/                              # generated by pipeline.R (gitignored)
│   ├── learning_curve/                 # LSTM training-history plots (*_learning.png)
