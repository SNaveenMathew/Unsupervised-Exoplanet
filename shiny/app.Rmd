...
server = function(input, output) {
  ...
  starts_ends <- reactive({
    ...
    if(nrow(query_result) == 0) {
      ...
      # query <- paste0("SELECT * FROM test_idx WHERE id=", username, ";")
      # query_result <- dbGetQuery(mydb, query)
      # insert_query <- paste0("INSERT INTO user_star ('", username, "', ", id, ", 0, 0);")
  ...
  getImage <- reactive({
    ...
    # files <- list.files("../plots/test_pred_plot/", pattern = input$kplr_id,
                        # full.names = T)
    # print(input$trainPlot_brush)
    if(length(files) > 0) {
      file <- files[1]
      img <- readPNG(file)
      file.remove("temp_png.png")
  ...
  output$planet_page <- renderUI({
    req(credentials()$user_auth)
    return(fluidPage(
      plotOutput("trainPlot", brush = brushOpts(id = "trainPlot_brush", direction = 'x'))
    ))
  })
})
